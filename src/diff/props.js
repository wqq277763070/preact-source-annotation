...
function setProperty(dom, name, value, oldValue, isSvg) {
	//如果是svg，属性名className要转为class，非svg时class要转className
	if (isSvg) {
		if (name === 'className') {
			name = 'class';
		}
	...
	if (name === 'key' || name === 'children') {
		...
		if (typeof value == 'string') {
			...
			//如果oldValue中的属性名不在newValue中则设置空
			if (oldValue) {
			...
			//如果没有老的属性值或者新老属性值不相等，则调用setStyle设置样式
			if (value) {
				for (let i in value) {
	...
	//如果属性名前面为on,则为事件
		...
		if (value) {
			//只有老值为非才添加事件，因为事件函数是固定的eventProxy，实际的事件在listeners储存
	...
	//属性名不为list，tabName等并且非svg并且属性名在dom中，直接设置给dom
	else if (
		name !== 'list' &&
		name !== 'tagName' &&
		...
		//属性值为null或者为false且属性名不是以ar开头 则移除属性
		else if (
			value == null ||
			(value === false &&
